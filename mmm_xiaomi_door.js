...
Module.register('mmm_xiaomi_door', {
  ...
  html: {
    // For text layout
    table: '<table class="large">{0}</table>',
    // Row parameters: 0: room name, 1: temperature, 2: humidity
    col: '<td align="left" class="normal light large">{0}</td>',
    ...
    roomDiv: '<div class="xm-room {0} normal light large"><div class="xm-room-header medium">{1}</div><div class="xm-room-temp-humid"><div class="xm-room-temp">{2}°C</div><div class="xm-room-humid">{3}%</div></div><div class="xm-room-icons"><div class="xm-door-icon">{4}</div><div class="xm-light-icon">{5}</div><div class="xm-vent-icon">{6}</div></div></div>',
      ...
      $.each(data, function (i, room) {
        if (room != null) {
          ...
          if (this.config.showWindow) {
            ...
            var windowIcon = (windowOpen.length > 0) ? "Nyitva" : "Zárva";
